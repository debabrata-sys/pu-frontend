...
import {
  ...
  Grid, IconButton, Paper, FormControl, InputLabel, Select, MenuItem,
...
export default function CreateFeedbackInternal() {
  ...
  const fetchFeedback = async () => {
    try {
      const res = await ep1.get("/api/v2/getsinglefeedbackinternalds", {
        params: { feedbackId }
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const endpoint = isEdit 
        ? `/api/v2/updatefeedbackinternalds?feedbackId=${feedbackId}`
        : "/api/v2/createfeedbackinternalds";
      
      const params = isEdit ? {} : { colid: global1.colid };
      
      await ep1.post(endpoint, formData, { params });
      navigate('/feedbackinternalmanagement');
    } catch (error) {
    }
    setLoading(false);
  };
        ...
        <CardContent sx={{ p: 0 }}>
          ...
          >
            ...
            <Typography variant="h5" fontWeight={600}>
              {isEdit ? "✏️ Edit Internal Feedback" : "➕ Create Internal Feedback"}
            </Typography>
          </Box>

          <Box sx={{ p: 4 }}>
            {/* Auto-Prefilled Filter Info */}
            {(prefilledData.programcode || prefilledData.coursecode || prefilledData.year || prefilledData.semester) && (
              <Paper 
                elevation={1} 
                sx={{ 
                  p: 3, 
              ...
              >
                ...
                <Grid container spacing={2}>
                  {prefilledData.programcode && (
                    <Grid item>
                      <Chip label={`Program: ${prefilledData.programcode}`} color="success" />
                    </Grid>
                  )}
                  {prefilledData.coursecode && (
                    <Grid item>
                      <Chip label={`Course: ${prefilledData.coursecode}`} color="success" />
                    </Grid>
                  )}
                  {prefilledData.year && (
                    <Grid item>
                      <Chip label={`Year: ${prefilledData.year}`} color="success" />
                    </Grid>
                  )}
                  {prefilledData.semester && (
                    <Grid item>
                      <Chip label={`Semester: ${prefilledData.semester}`} color="success" />
                    </Grid>
                  )}
                </Grid>
              </Paper>
            ...
            {formData.questions.map((question, questionIndex) => (
              ...
              >
                ...
                <Grid container spacing={2}>
                  ...
                  {/* Preview Section */}
                  <Grid item xs={12}>
                    <Box sx={{ p: 2, backgroundColor: '#f8f9fa', borderRadius: 1, border: '1px dashed #ddd' }}>
            ...
            <Box display="flex" gap={2} justifyContent="flex-end">
              <Button
                variant="outlined"
                onClick={() => navigate('/feedbackinternalmanagement')}
              ...
              >
                {loading ? "Saving..." : (isEdit ? "Update Feedback" : "Create Feedback")}
              </Button>
            </Box>
          </Box>
        </CardContent>

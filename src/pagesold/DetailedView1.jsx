...
export default function DetailedView1() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ep1
      .get(`/api/v2/getsinglerubrics1?id=${id}&colid=${colid}`)
  ...
  const handleDelete = () => {
    ep1
      .get(`/api/v2/deleterubrics1?id=${id}`) // Fixed endpoint name
      .then(() => {
  ...
  const handleFinalizeExam = async () => {
    try {
      ...
      const resultResponse = await ep1.get("/api/v2/calculaterubricresult1", {
      ...
      if ([200, 201].includes(createResponse.status)) {
        ...
        setSnackbarSeverity("success");
        setOpenSnackbar(true);
        setTimeout(() => navigate("/finalizedata1"), 2000);
      } else {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      {/* Marks Sections - Only Internal Marks */}
      {renderMarks(

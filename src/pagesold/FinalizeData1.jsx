...
export default function FinalizeData1() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await ep1.get("/api/v2/getallrubrics1", {
  ...
  const handleBulkFinalize = async () => {
    ...
    try {
      ...
      const calcRes = await ep1.get("/api/v2/calculatebulkrubricresults1", {
        params: { ...filters, colid: global1.colid },
      });
    ...
    { field: "programcode", headerName: "Program Code", width: 130 },
    {
      ...
      renderCell: ({ row }) => (
        <Box display="flex" gap={1}>
          ...
          <Button variant="outlined" size="small" onClick={() => navigate(`/detailedview1/${row._id}`)}>
            View
          </Button>
        </Box>
      ),

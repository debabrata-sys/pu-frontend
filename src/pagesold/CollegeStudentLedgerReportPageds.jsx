...
const CollegeStudentLedgerReportPageds = () => {
  ...
  const years = ['2021-22', '2022-23', '2023-24', '2024-25', '2025-26', '2026-27', '2027-28', '2028-29'];
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (programcode && programcode.trim()) url += `&programcode=${encodeURIComponent(programcode.trim())}`;
      if (academicyear && academicyear.trim()) url += `&academicyear=${encodeURIComponent(academicyear.trim())}`;
      if (semester && semester.trim()) url += `&semester=${encodeURIComponent(semester.trim())}`;
      if (classdate && classdate.trim()) url += `&classdate=${encodeURIComponent(classdate.trim())}`;
      ...
      if (response.data.success) {
        ...
        if (!response.data.data.transactions || response.data.data.transactions.length === 0) {
          setError('No records found for the specified filters. Try different criteria.');
        }
      }
    } catch (err) {
      console.error("Error:", err);
      setError(err.response?.data?.message || 'Error generating report. Please check filters and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportToExcel = () => {
    ...
    const excelData = [
      ...
      [''],
      ['STUDENT SUMMARY'],
      ['Student Name', 'Reg No', 'Program Code', 'Semester', 'Total Payable', 'Total Paid', 'Balance', 'Transactions'],
      ...
      ['Date', 'Student Name', 'Reg No', 'Fee Item', 'Category', 'Amount', 'Type', 'Payment Mode', 'Status'],
      ...ledgerData.map((txn) => [
  ...
  return (
    <Box sx={{ p: 3, bgcolor: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
        ...
        {(programcode || academicyear || semester || classdate) && (
          <Box sx={{ mb: 2, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            ...
            {classdate && (
              <Chip
                label={`Due Date: ${new Date(classdate).toLocaleDateString('en-IN')}`}
        ...
        {error && (
          <Alert 
            severity={reportGenerated && (!ledgerData || ledgerData.length === 0) ? "info" : "error"} 
            sx={{ mt: 2 }}
          >
            {error}
          </Alert>
      ...
      {/* Summary Cards */}
      {overallSummary && reportGenerated && ledgerData && ledgerData.length > 0 && (
        <Grid container spacing={2} sx={{ mb: 3 }}>
            ...
            <Card sx={{ boxShadow: 2 }}>
              <CardContent sx={{ textAlign: 'center', p: 2 }}>
                ...
                <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#ff9800' }}>
                  ₹ {overallSummary.totalPayable.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                </Typography>
              </CardContent>
            </Card>
          </Grid>
            ...
            <Card sx={{ boxShadow: 2 }}>
              <CardContent sx={{ textAlign: 'center', p: 2 }}>
                ...
                <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#4caf50' }}>
                  ₹ {overallSummary.totalPaid.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                </Typography>
              </CardContent>
            </Card>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={2.4}>
            <Card sx={{ boxShadow: 2 }}>
              <CardContent sx={{ textAlign: 'center', p: 2 }}>
                ...
                >
                  ₹ {overallSummary.balance.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                </Typography>
              </CardContent>
            </Card>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={2.4}>
            <Card sx={{ boxShadow: 2 }}>
              <CardContent sx={{ textAlign: 'center', p: 2 }}>
                <Typography variant="body2" sx={{ color: '#666', mb: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
      ...
      {reportGenerated && ledgerData && ledgerData.length > 0 && (
        <Paper sx={{ borderRadius: 2, boxShadow: 2 }}>
                      ...
                      {studentSummary && studentSummary.length > 0 ? (
                        studentSummary.map((student, index) => (
                          <TableRow key={index} hover>
                            ...
                            <TableCell>{student._id.programcode || '-'}</TableCell>
                            <TableCell>{student._id.semester || '-'}</TableCell>
                            <TableCell align="right" sx={{ color: '#ff9800', fontWeight: 'bold' }}>
                              ₹ {student.totalPayable.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </TableCell>
                            <TableCell align="right" sx={{ color: '#4caf50', fontWeight: 'bold' }}>
                              ₹ {student.totalPaid.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </TableCell>
                            <TableCell
                              align="right"
                              sx={{
                                color: student.balance > 0 ? '#f44336' : '#4caf50',
                                fontWeight: 'bold',
                              }}
                            >
                              ₹ {student.balance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </TableCell>
                            <TableCell align="center">{student.transactionCount}</TableCell>
                          </TableRow>
                        ))
                      ) : (
                        <TableRow>
                    ...
                    <TableBody>
                      {ledgerData && ledgerData.length > 0 ? (
                        ledgerData.map((txn, index) => (
                          <TableRow key={index} hover>
                            <TableCell>{new Date(txn.classdate).toLocaleDateString('en-IN')}</TableCell>
                            ...
                            <TableCell>
                              <Chip label={txn.status || 'Pending'} size="small" />
                            </TableCell>
                          </TableRow>
                        ))
                      ) : (
          ...
          {overallSummary && (
            <Box sx={{ p: 3, borderTop: '2px solid #ddd', bgcolor: '#fafafa' }}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={2.4}>
                  <Box sx={{ textAlign: 'right' }}>
                    <Typography variant="body2" sx={{ color: '#666', mb: 0.5, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 0.5 }}>
                      <People sx={{ fontSize: 16 }} />
                      Students
                    </Typography>
                    <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#9c27b0' }}>
          ...
          {/* Footer */}
          <Box sx={{ p: 2, borderTop: '1px solid #ddd', textAlign: 'center', bgcolor: '#fafafa' }}>

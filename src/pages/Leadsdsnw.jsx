...
const Leadsds = () => {
  ...
  const customFieldColumns = getUniqueCustomFields().map(fieldName => ({
    field: `custom_${fieldName}`,
    headerName: fieldName,
    width: 150,
    editable: true,
    valueGetter: (value, row) => {
      if (!row.custom_fields) return "";
      const field = row.custom_fields.find(f => f.field_name === fieldName);
      return field ? field.field_value : "";
    },
    valueSetter: (value, row) => {
      const customFields = row.custom_fields || [];
      const existingFieldIndex = customFields.findIndex(f => f.field_name === fieldName);

      if (existingFieldIndex >= 0) {
        customFields[existingFieldIndex].field_value = value;
      } else {
        customFields.push({ field_name: fieldName, field_value: value });
      }

      return { ...row, custom_fields: customFields };
    }
  }));
  ...
  const baseColumns = [
    ...
    { field: "course_interested", headerName: "Course", width: 150, editable: true },
    {
    ...
    { field: "source", headerName: "Source", width: 120, editable: true },

...
export default function ConductExam2ComponentwiseMarksPage() {
  ...
  const loadPapers = async () => {
    try {
      ...
      setError("");
      setMessage("");
      const res = await ep1.get("/api/v2/conductexam2/component-marks-papers", {
  ...
  const loadRows = async (paper = selectedPaper, nextFilters = filters) => {
    ...
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/component-marks-rows", { params });
      const data = res.data?.data || [];
      setRows(data);
  ...
  const saveMarks = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      const res = await ep1.post("/api/v2/conductexam2/component-marks-savebulk", {
        colid: global1.colid,
        user: examinerEmail,
        rows: payloadRows
      });
  ...
  const submitMarks = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      const res = await ep1.post("/api/v2/conductexam2/component-marks-submit", {
        colid: global1.colid,
        user: examinerEmail,
        rows: payloadRows
      });
  ...
  return (
    <MenuPageShell title="Componentwise Marks 2">
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            <Box>
              <Typography variant="h5" fontWeight={900}>Componentwise Marks 2</Typography>

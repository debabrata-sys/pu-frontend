...
export default function StudentOnlineFeePaymentPage() {
  ...
  const startPayment = async () => {
    ...
    try {
      console.log("[StudentOnlineFeePayment] Creating payment session with payload:", {
        colid,
        regno,
      ...
      console.log("[StudentOnlineFeePayment] Session created response:", session);
      ...
      if (selectedGateway.type === "External" && !gatewayName.includes("easebuzz") && !gatewayName.includes("icici")) {
        ...
        console.log("[StudentOnlineFeePayment] Redirecting to external gateway link:", `${selectedGateway.externallink}${joiner}${params.toString()}`);
      ...
      console.log("[StudentOnlineFeePayment] Calling gateway initiate API:", endpoint, initiatePayload);

      const payRes = await ep1.post(endpoint, initiatePayload);
      console.log("[StudentOnlineFeePayment] Gateway initiate API response:", payRes.data);

      const paymentUrl = payRes.data?.data?.paymenturl || payRes.data?.paymenturl || payRes.data?.data?.paymentUrl || payRes.data?.paymentUrl;
      console.log("[StudentOnlineFeePayment] Extracted Payment Redirect URL:", paymentUrl);
      if (!paymentUrl) throw new Error("Gateway did not return a payment URL.");
      window.location.assign(paymentUrl);
    } catch (err) {
      console.error("[StudentOnlineFeePayment] Payment initiation error:", err.response?.data || err);

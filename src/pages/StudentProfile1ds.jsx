...
const StudentProfile1ds = () => {
    ...
    const [rubricData, setRubricData] = useState([]); // Now stores exammarks2ds data
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                // Use getexammarks2ds instead of getrubricds
                const [
                    userRes,
                    examRes,
                    appFormRes,
                    ...
                    ep1.get(`/api/v2/getexammarks2ds?colid=${colid}&regno=${regno}`),
                ]);

                console.log('ExamMarks2DS Response:', rubricRes.data);
                ...
                // console.error(err);
                // It's okay if some data is missing, but log it
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <Box sx={{ p: isSmallScreen ? 2 : 4, maxWidth: 1200, mx: "auto" }}>
            ...
            <Grid container direction="column" rowSpacing={4}>
                            ...
                            {examMarksData.map((exam, idx) => (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            {`${exam.coursecode || "Course Code"} - ${exam.course || "Course"
                                                } (${exam.semester || ""})`}
                                        </Typography>
                                    </AccordionSummary>
                                    <AccordionDetails sx={{ px: 3, pb: 2 }}>
                ...
                {/* Rubric Marks Section - REFACTORED for exammarks2ds */}
                {rubricData.length > 0 && (
                    <Grid item xs={12}>
                        <ProfileCard
                            title="Rubric Marks Details (Updated)"
                            ...
                            {rubricData.map((rubric, idx) => (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            {`${rubric.papercode || ""} - ${rubric.papername || "Paper"} (${rubric.semester})`}
                                        </Typography>
                                    </AccordionSummary>
                                    <AccordionDetails sx={{ px: 3, pb: 2 }}>
                                        <Grid container spacing={2}>
                                            <Grid item xs={12} md={6}>
                                                <Typography variant="subtitle2" color="primary">Theory Marks</Typography>
                                                <Typography>Obtained: {rubric.thobtained} / {rubric.thmax}</Typography>
                                            </Grid>
                                            <Grid item xs={12} md={6}>
                                                <Typography variant="subtitle2" color="primary">Practical Marks</Typography>
                                                <Typography>Obtained: {rubric.probtained} / {rubric.prmax}</Typography>
                                            </Grid>
                                            <Grid item xs={12} md={6}>
                                                <Typography variant="subtitle2" color="primary">Internal (Theory)</Typography>
                                                <Typography>Obtained: {rubric.iatobtained} / {rubric.iatmax}</Typography>
                                            </Grid>
                                            <Grid item xs={12} md={6}>
                                                <Typography variant="subtitle2" color="primary">Internal (Practical)</Typography>
                                                <Typography>Obtained: {rubric.iapobtained} / {rubric.iapmax}</Typography>
                                            </Grid>
                                            <Grid item xs={12}>
                                                <Divider sx={{ my: 1 }} />
                                                <Typography variant="subtitle1" fontWeight="bold">Result: {rubric.status || "N/A"}</Typography>
                                            </Grid>
                                        </Grid>
                                    </AccordionDetails>
                                </Accordion>
...
export default StudentProfile1ds;

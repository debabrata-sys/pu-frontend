...
export default function ConductExam2PaperSetterRegistrationPage() {
  ...
  const loadOptions = async (params = {}) => {
    const res = await ep1.get("/api/v2/conductexam2/papersetter-options", { params: { colid: global1.colid, ...params } });
  ...
  const loadRows = async (nextFilters = filters) => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/papersetters", { params });
  ...
  const savePaperSetter = async () => {
    try {
      ...
      if (!editId && selectedUsers.length) {
        ...
        const res = await ep1.post("/api/v2/conductexam2/papersetters-bulk", { colid: global1.colid, user: global1.user, items });
        ...
        const res = await ep1.post("/api/v2/conductexam2/papersetters", { ...base, id: editId });
        const emailMsg = res.data?.emailResult?.emailSent
          ? ` Appointment letter with login credentials sent to ${base.papersetteremail}.`
          : (res.data?.emailResult?.message ? ` (${res.data.emailResult.message})` : "");
        setMessage((editId ? "Paper setter updated." : "Paper setter saved.") + emailMsg);
      }
      setForm(blankForm);
      setAdminDocTitle("");
      setSelectedUsers([]);
  ...
  const deleteRow = async (id) => {
    ...
    await ep1.post("/api/v2/conductexam2/papersetters-delete", { id, colid: global1.colid });
  ...
  const sendAppointmentLetter = async (row) => {
    try {
      setMessage(`Sending appointment letter with login credentials to ${row.papersetteremail || row.papersettername}...`);
      const res = await ep1.post("/api/v2/conductexam2/papersetters-send-letter", { colid: global1.colid, id: row._id });
      if (res.data?.success) {
        setMessage(res.data.message || `Appointment letter sent to ${row.papersetteremail}.`);
      } else {
        setError(res.data?.message || "Failed to send appointment letter.");
      }
    } catch (err) {
      setError(err.response?.data?.message || "Unable to send appointment letter.");
    }
  };
  ...
  const columns = [
    ...
    { field: "actions", headerName: "Actions", width: 270, sortable: false, renderCell: (params) => <Stack direction="row" spacing={1}><Button size="small" variant="outlined" color="primary" onClick={() => sendAppointmentLetter(params.row)}>Send Letter</Button><Button size="small" onClick={() => editRow(params.row)}>Edit</Button><Button size="small" color="error" onClick={() => deleteRow(params.row._id)}>Delete</Button></Stack> }
  ];

...
const useConductExamOptions = () => {
  ...
  const load = async () => {
    setBusy(true);
    try {
      const res = await ep1.get("/api/v2/conductexam2/moderator-options", { params: { colid: global1.colid } });
...
const usePanelOptions = () => {
  ...
  const loadPanels = async (params = {}) => {
    const res = await ep1.get("/api/v2/conductexam2/moderator-panels", { params: { colid: global1.colid, ...params } });
...
export function ConductExam2ModeratorPanelPage() {
  ...
  const loadRows = async () => {
    setLoading(true);
    try {
      const res = await ep1.get("/api/v2/conductexam2/moderator-panels", { params: { colid: global1.colid, ...filterParams(filters) } });
  ...
  const save = async () => {
    setSaving(true);
    try {
      await ep1.post("/api/v2/conductexam2/moderator-panels", { ...form, id: editId, colid: global1.colid, name: global1.name, user: global1.user });
  ...
  const bulkDelete = async () => {
    ...
    setSaving(true);
    try {
      await ep1.post("/api/v2/conductexam2/moderator-panels-delete", { colid: global1.colid, ids: selected });
  ...
  return (
    <MenuPageShell title="Moderator Panel">
      <Box sx={{ p: 3, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            <Box><Typography variant="h5" fontWeight={900}>Moderator Panel</Typography><Typography color="text.secondary">Create program-wise moderator panels.</Typography></Box>
            <Stack direction="row" spacing={1}>
              <Button variant="outlined" onClick={() => XLSX.writeFile(Object.assign(XLSX.utils.book_new(), { Sheets: { Template: XLSX.utils.json_to_sheet([{ ...blankPanel, academicyear: "2026-27", regulation: "Regulation", program: "Program", programcode: "CODE", panelname: "Panel 1" }]) }, SheetNames: ["Template"] }), "moderator_panel_template.xlsx")}>Template</Button>
              <Button component="label" variant="contained" startIcon={<UploadFileIcon />} disabled={uploading}>Bulk Upload<input hidden type="file" accept=".xlsx,.xls,.csv" onChange={(e) => uploadSheet(e, (row) => row, "/api/v2/conductexam2/moderator-panels-bulk", (data) => { setMessage(`${data.saved || 0} panels uploaded.`); loadRows(); }, setError, setUploading)} /></Button>
...
export function ConductExam2ModeratorPanelAssignmentPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/moderator-panel-members", { params: { colid: global1.colid, ...filterParams(filters) } });
  ...
  const assign = async () => {
    ...
    setSaving(true);
    try {
      await ep1.post("/api/v2/conductexam2/moderator-panel-members", { colid: global1.colid, panelid: selectedPanel._id, name: global1.name, user: global1.user, users: selectedUsers.map((u) => ({ membername: u.name, memberemail: u.email, role: u.role, department: u.department, designation: u.designation, institution: u.institution })) });
  ...
  const bulkDelete = async () => {
    if (!selected.length || !window.confirm("Delete selected moderator panel members?")) return;
    setSaving(true);
    try {
      await ep1.post("/api/v2/conductexam2/moderator-panel-members-delete", { colid: global1.colid, ids: selected });
  ...
  const downloadTemplate = () => {
    ...
    XLSX.writeFile(wb, "moderator_panel_members_template.xlsx");
  };

  return (
    <MenuPageShell title="Moderator Panel Assignment">
      <Box sx={{ p: 3, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            <Box><Typography variant="h5" fontWeight={900}>Moderator Panel Assignment</Typography><Typography color="text.secondary">Add users to a panel. Member approval is handled separately.</Typography></Box>
            <Stack direction="row" spacing={1}>
              ...
              <Button component="label" variant="contained" startIcon={<UploadFileIcon />} disabled={uploading || !selectedPanel}>
                ...
                <input hidden type="file" accept=".xlsx,.xls,.csv" onChange={async (e) => {
                  ...
                  try {
                    ...
                    const res = await ep1.post("/api/v2/conductexam2/moderator-panel-members", { colid: global1.colid, panelid: selectedPanel._id, name: global1.name, user: global1.user, users });
...
export function ConductExam2ModeratorPanelApprovalPage() {
  ...
  const act = async (approvalstatus) => {
    ...
    setSaving(true);
    try {
      await ep1.post("/api/v2/conductexam2/moderator-panel-members-approve", { colid: global1.colid, ids: selected, approvalstatus, comments, approvedby: global1.name, approvedbyemail: global1.user });
  ...
  return (
    <MenuPageShell title="Moderator Panel Approval">
      <Box sx={{ p: 3, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}><Typography variant="h5" fontWeight={900}>Moderator Panel Approval</Typography><Typography color="text.secondary">Approve or reject selected moderator panel members. The panel itself is not approved.</Typography>{(loading || saving) && <LinearProgress sx={{ mt: 2 }} />}</Paper>
        {message && <Alert severity="success" sx={{ mb: 2 }} onClose={() => setMessage("")}>{message}</Alert>}
        {error && <Alert severity="error" sx={{ mb: 2 }} onClose={() => setError("")}>{error}</Alert>}
...
export function ConductExam2ModeratorRegistration2Page() {
  const { courses } = useConductExamOptions();
  const { panels, loadPanels } = usePanelOptions();
  ...
  const loadMembers = async (panel) => {
    ...
    if (!panel?._id) return;
    const res = await ep1.get("/api/v2/conductexam2/moderator-panel-members", { params: { colid: global1.colid, panelid: panel._id, approvalstatus: "Approved", status: "Active" } });
  ...
  const loadRows = async () => {
    setLoading(true);
    try {
      const res = await ep1.get("/api/v2/conductexam2/moderators", { params: { colid: global1.colid, ...filterParams(filters) } });
      setRows(res.data?.data || []);
    } catch (err) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await ep1.post("/api/v2/conductexam2/moderators", { ...form, id: editId, moderatorname: selectedMember.membername, moderatoremail: selectedMember.memberemail, colid: global1.colid, user: global1.user });
      setMessage(editId ? "Moderator registration updated." : "Moderator registered from approved panel member.");
  ...
  const uploadAdminDocument = async (file) => {
    ...
    try {
      ...
      const res = await ep1.post("/api/v2/conductexam2/question-paper-upload", formData, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const bulkDelete = async () => {
    if (!selected.length || !window.confirm("Delete selected moderator registrations?")) return;
    setSaving(true);
    try {
      await Promise.all(selected.map((id) => ep1.post("/api/v2/conductexam2/moderators-delete", { id, colid: global1.colid })));
  ...
  return (
    <MenuPageShell title="Moderator Registration 2">
      <Box sx={{ p: 3, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}><Box><Typography variant="h5" fontWeight={900}>Moderator Registration 2</Typography><Typography color="text.secondary">Select a panel first, then register only approved panel members as moderators.</Typography></Box><Button color="error" variant="outlined" disabled={!selected.length || saving} onClick={bulkDelete}>Bulk Delete</Button></Stack>

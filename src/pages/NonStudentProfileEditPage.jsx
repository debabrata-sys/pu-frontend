      ...
      <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
        <Box>
          <Breadcrumbs sx={{ mb: 0.5 }}>
            ...
            <Typography color="text.primary">Non-Student Profile</Typography>
          </Breadcrumbs>
...
export default function NonStudentProfileEditPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Load document requirements configured at /userprofiledetailrequirements
      const [reqRes, uploadRes] = await Promise.all([
        ep1.get("/api/v2/user-profile-detail-requirements", {
          params: { colid: global1.colid, role, status: "Active" }
        }),
        ep1.get("/api/v2/user-uploaded-documents", {
          params: { colid: global1.colid, role, owneruser: global1.user }
        })
      ]);
      setDocumentRequirements(reqRes.data?.data || []);
      setDocumentUploads(uploadRes.data || []);
    } catch (err) {
      setError(err.response?.data?.msg || err.response?.data?.message || "Unable to load profile");
    } finally {
      setLoading(false);
    }
  };
  ...
  const uploadDocument = async (doc) => {
    ...
    try {
      ...
      setError(err.response?.data?.msg || err.response?.data?.message || "Unable to upload document");
    } finally {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Stack direction="row" spacing={1} alignItems="center" sx={{ width: "100%" }}>
          ...
          <Typography variant="caption" noWrap sx={{ width: 120 }}>
  ...
  const uploadColumns = [
    ...
    },
    {
      ...
      width: 140,
      renderCell: (params) => {
        const val = params.value || "Uploaded";
  ...
  return (
    <MenuPageShell title="Non-Student Profile Edit">
      <Box p={3} sx={{ bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <PageTop title="Non-Student Profile Edit" subtitle="View, update fields and upload required documents configured for your role." />
        ...
        >
          Data compliance notice: profile data is processed for institutional services, statutory compliance, employment records, and communication. Consent status: {consentGiven ? `Active${consentStatus?.latest?.activitytime ? ` since ${new Date(consentStatus.latest.activitytime).toLocaleString()}` : ""}` : "Not given. Profile editing is inactive until consent is recorded."}
        </Alert>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems={{ xs: "stretch", sm: "center" }} justifyContent="space-between">
            <Stack direction="row" spacing={2} alignItems="center">
              ...
              <Box>
                ...
                <Typography color="text.secondary">Upload JPG, JPEG or PNG profile photo.</Typography>
        ...
        <Paper sx={{ p: 2, mt: 2 }}>
          <Typography variant="h6" fontWeight={800} sx={{ mb: 1 }}>Profile Edit Approval Status</Typography>
        ...
        <Paper sx={{ p: 1, mt: 2, borderRadius: 2, overflowX: "auto" }}>
          <Typography variant="h6" fontWeight={800} sx={{ p: 1 }}>Configured Document Requirements (from /userprofiledetailrequirements)</Typography>
          {!documentRequirements.length && <Alert severity="info" sx={{ m: 1 }}>No document requirements configured for role {role}.</Alert>}
          <DataGrid
        ...
        <Paper sx={{ p: 2, mt: 2, borderRadius: 2 }}>
          <Typography variant="h6" fontWeight={800} sx={{ mb: 1 }}>Document Approval Status</Typography>

...
export default function ConductExam2ModeratorRegistrationPage() {
  ...
  const loadOptions = async (params = {}) => {
    const res = await ep1.get("/api/v2/conductexam2/moderator-options", { params: { colid: global1.colid, ...params } });
  ...
  const loadRows = async (nextFilters = filters) => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/moderators", { params });
  ...
  const saveModerator = async () => {
    try {
      ...
      if (!editId && selectedUsers.length) {
        ...
        const res = await ep1.post("/api/v2/conductexam2/moderators-bulk", { colid: global1.colid, user: global1.user, items });
        ...
        const res = await ep1.post("/api/v2/conductexam2/moderators", { ...base, id: editId });
        const emailMsg = res.data?.emailResult?.emailSent
          ? ` Appointment letter with login credentials sent to ${base.moderatoremail}.`
          : (res.data?.emailResult?.message ? ` (${res.data.emailResult.message})` : "");
        setMessage((editId ? "Moderator updated." : "Moderator saved.") + emailMsg);
      }
      setForm(blankForm);
      setAdminDocTitle("");
      setSelectedUsers([]);
  ...
  const deleteRow = async (id) => {
    ...
    await ep1.post("/api/v2/conductexam2/moderators-delete", { id, colid: global1.colid });
  ...
  const sendAppointmentLetter = async (row) => {
    try {
      setMessage(`Sending appointment letter with login credentials to ${row.moderatoremail || row.moderatorname}...`);
      const res = await ep1.post("/api/v2/conductexam2/moderators-send-letter", { colid: global1.colid, id: row._id });
      if (res.data?.success) {
        setMessage(res.data.message || `Appointment letter sent to ${row.moderatoremail}.`);
      } else {

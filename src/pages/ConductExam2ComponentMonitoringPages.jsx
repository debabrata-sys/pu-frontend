...
function useMonitoringOptions() {
  ...
  useEffect(() => {
    ep1.get("/api/v2/conductexam2/component-monitoring-options", { params: { colid: global1.colid } })
      .then((res) => {
...
export function ConductExam2MarksEntryMonitoringPage() {
  ...
  const load = async () => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/component-marks-monitoring", { params });
  ...
  return (
    <MenuPageShell title="Marks Entry Monitoring 2">
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            ...
            <Button variant="outlined" onClick={() => printRefContent(printRef, "Marks Entry Monitoring 2")} disabled={!rows.length}>Print Preview</Button>
          </Stack>
        </Paper>
        ...
        <PrintBlock printRef={printRef} title="Marks Entry Monitoring 2" filters={filters} institution={institution} rows={rows} columns={columns} totals={totals} />
      </Box>
    </MenuPageShell>
  );
}

export function ConductExam2DaywiseMarksMonitoringPage() {
  const { options, optionsError } = useMonitoringOptions();
  const [filters, setFilters] = useState(blankFilters);
  const [rows, setRows] = useState([]);
  ...
  const load = async () => {
    try {
      ...
      const params = { colid: global1.colid };
      Object.entries(filters).forEach(([key, value]) => { if (value) params[key] = value; });
      const res = await ep1.get("/api/v2/conductexam2/component-marks-daywise-monitoring", { params });
      setRows(res.data?.data || []);
  ...
  return (
    <MenuPageShell title="Examiner Monitoring 2">
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            ...
            <Button variant="outlined" onClick={() => printRefContent(printRef, "Examiner Monitoring 2")} disabled={!rows.length}>Print Preview</Button>
          </Stack>
        </Paper>
        {(error || optionsError) && <Alert severity="error" sx={{ mb: 2 }} onClose={() => setError("")}>{error || optionsError}</Alert>}
        <FilterPanel filters={filters} setFilters={setFilters} options={options} onLoad={load} loading={loading} />
        ...
        <PrintBlock printRef={printRef} title="Examiner Monitoring 2" filters={filters} institution={institution} rows={rows} columns={columns} totals={totals} />
      </Box>
    </MenuPageShell>
  );
}

export function ConductExam2ExaminerReassignmentPage() {
  const { options, optionsError } = useMonitoringOptions();
  const [filters, setFilters] = useState(blankFilters);
  ...
  const load = async () => {
    try {
      ...
      const params = { colid: global1.colid };
      Object.entries(filters).forEach(([key, value]) => { if (value) params[key] = value; });
      const res = await ep1.get("/api/v2/conductexam2/component-reassignment-rows", { params });
  ...
  const reassign = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const res = await ep1.post("/api/v2/conductexam2/component-reassign-examiner", { colid: global1.colid, user: global1.user, ids: selectedIds, examinername: target.examinername, examineremail: target.examineremail });
  ...
  return (
    <MenuPageShell title="Examiner Reassignment 2">
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>

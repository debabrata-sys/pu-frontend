...
const JobManagerPage = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await ep1.get("/api/v2/deletejob", { params: { id } });
      fetchJobs();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editMode) {
        await ep1.post(`/api/v2/updatejob?id=${editId}`, payload);
        setSnack({ open: true, msg: "Job updated", severity: "success" });
      } else {
        await ep1.post("/api/v2/createjob", payload);
      ...
      <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
        ...
        >
          ...
          <Stack direction="row" spacing={2}>
            ...
            <Button variant="contained" onClick={()=> navigate('/allcvpage')}>
              Search CV
            </Button>
          </Stack>
        </Box>
        ...
        <Grid container spacing={3}>
          {jobs.map((job) => (
            <Grid item xs={12} sm={6} md={4} key={job._id}>
              ...
              variant="outlined"
              startIcon={<Group />}
              onClick={() => navigate(`/applications-status/${job._id}`)}
            >
              Manage Applications
            </Button>
                </CardContent>

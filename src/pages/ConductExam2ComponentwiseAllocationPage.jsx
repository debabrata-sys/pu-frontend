...
export default function ConductExam2ComponentwiseAllocationPage() {
  ...
  const loadOptions = async () => {
    const res = await ep1.get("/api/v2/conductexam2/component-allocation-options", { params: { colid: global1.colid } });
  ...
  const loadRows = async (nextFilters = filters) => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/component-allocations", { params });
  ...
  const loadPresentStudents = async () => {
    try {
      setLoading(true);
      setError("");
      const res = await ep1.get("/api/v2/conductexam2/component-allocation-students", { params: { colid: global1.colid, ...form } });
  ...
  const saveManual = async () => {
    try {
      setSaving(true);
      setError("");
      await ep1.post("/api/v2/conductexam2/component-allocations", { ...form, id: editId, colid: global1.colid, user: global1.user });
  ...
  const randomAllocate = async (useLimit = false, useAi = false) => {
    ...
    try {
      ...
      setError("");
      setAiResponse("");
      const res = await ep1.post("/api/v2/conductexam2/component-allocations-random", {
        ...form,
        colid: global1.colid,
        user: global1.user,
  ...
  const deleteRows = async (ids) => {
    ...
    await ep1.post("/api/v2/conductexam2/component-allocations-bulk-delete", { colid: global1.colid, ids });
  ...
  const uploadExcel = async (event) => {
    ...
    try {
      ...
      const res = await ep1.post("/api/v2/conductexam2/component-allocations-bulk", { colid: global1.colid, user: global1.user, rows: rowsFromFile });
  ...
  return (
    <MenuPageShell title="Componentwise Allocation 2">
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            <Box>
              <Typography variant="h5" fontWeight={900}>Componentwise Allocation 2</Typography>

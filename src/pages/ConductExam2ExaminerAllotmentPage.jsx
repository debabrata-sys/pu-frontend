...
export default function ConductExam2ExaminerAllotmentPage() {
  ...
  const loadOptions = async () => {
    const res = await ep1.get("/api/v2/conductexam2/examiner-options", { params: { colid: global1.colid } });
  ...
  const loadRows = async (nextFilters = filters) => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/examiner-allotments", { params });
  ...
  const availableExaminers = useMemo(() => {
    ...
    // Tier 1: Registered specifically for this course & exam in ConductExamExaminer
    examiners.forEach((row) => {
      const email = (row.examineremail || "").toLowerCase().trim();
      if (!email || seenEmails.has(email)) return;
      const isMatch = (!form.academicyear || row.academicyear === form.academicyear)
      ...
      if (isMatch) {
        ...
        list.push({
          ...row,
          category: "Course Registered Examiners"
        });
      }
    });

    // Tier 2: Registered for this course under another exam/year
    if (form.coursecode) {
      examiners.forEach((row) => {
        const email = (row.examineremail || "").toLowerCase().trim();
        if (!email || seenEmails.has(email)) return;
        if (row.coursecode === form.coursecode) {
          seenEmails.add(email);
          list.push({
            ...row,
            category: "Course Examiners (Other Terms)"
          });
        }
      });
    }

    // Tier 3: Other registered examiners in ConductExamExaminer
    examiners.forEach((row) => {
      const email = (row.examineremail || "").toLowerCase().trim();
      if (!email || seenEmails.has(email)) return;
      seenEmails.add(email);
      list.push({
        ...row,
        category: "Other Registered Examiners"
      });
    });

    // Tier 4: Institutional Faculty from users
    users.forEach((u) => {
    ...
    return list;
  }, [examiners, users, form]);

  const hasCourseSpecificExaminers = useMemo(() => {
    if (!form.coursecode) return true;
    return examiners.some((row) => (
  ...
  const loadPresentStudents = async () => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/examiner-present-students", { params });
  ...
  const saveAllotment = async () => {
    ...
    try {
      setSaving(true);
      setError("");
      await ep1.post("/api/v2/conductexam2/examiner-allotments", { ...form, id: editId, colid: global1.colid, user: global1.user });
  ...
  const randomAllot = async (useLimit = false) => {
    ...
    try {
      ...
      const res = await ep1.post("/api/v2/conductexam2/examiner-allotments-random", payload);
      setRows(res.data?.data || []);
  ...
  const deleteRow = async (id) => {
    ...
    await ep1.post("/api/v2/conductexam2/examiner-allotments-delete", { id, colid: global1.colid });
    setMessage("Allotment deleted.");
    await loadRows();
  };

  const sendAppointmentLetter = async (row) => {
    try {
      setMessage("");
      setError("");
      const res = await ep1.post("/api/v2/conductexam2/examiner-allotments-send-letter", {
        id: row._id,
        colid: global1.colid
      });
      if (res.data?.success) {
        setMessage(`Appointment letter processed for ${row.examinername} (${row.examineremail}).`);
      } else {
        setError(res.data?.message || "Failed to send appointment letter.");
      }
    } catch (err) {
      setError(err.response?.data?.message || "Error sending appointment letter.");
    }
  };
  ...
  const bulkDeleteRows = async () => {
    ...
    try {
      ...
      setError("");
      const res = await ep1.post("/api/v2/conductexam2/examiner-allotments-bulk-delete", { ids: selectedIds, colid: global1.colid });
  ...
  const downloadTemplate = () => {
    ...
    const ws = XLSX.utils.json_to_sheet([{
      ...
      examinername: "Dr. Faculty",
      examineremail: "faculty@example.com",
      student: "John Doe",
      regno: "REG-001",
      email: "student@example.com",
      seatno: "A-101",
      examdate: "2026-06-15",
      examslot: "Morning (09:00 AM - 12:00 PM)",
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ExaminerAllotments");
    XLSX.writeFile(wb, "examiner_allotments_template.xlsx");
  };
  ...
  const handleBulkUpload = async (event) => {
    ...
    try {
      ...
      if (!raw.length) throw new Error("Uploaded file is empty");
      const items = raw.map((row, index) => ({ ...row, rowNumber: index + 2 }));
      const res = await ep1.post("/api/v2/conductexam2/examiner-allotments-bulk", { colid: global1.colid, user: global1.user, items });
      const errors = Array.isArray(res.data?.errors) ? res.data.errors : [];
      if (errors.length) {
        setError(`Saved ${res.data?.saved || 0} rows. Failed rows: ${errors.map((e) => `Row ${e.rowNumber}: ${e.message}`).join(", ")}`);
      } else {
        setMessage(`Successfully imported ${res.data?.saved || 0} allotment rows.`);
      }
      await loadRows();
    } catch (err) {
    ...
    { field: "evaluationdate", headerName: "Evaluation Date", width: 160 },
    {
      ...
      renderCell: (params) => (
        <Stack direction="row" spacing={1}>
          ...
          <Button size="small" color="primary" onClick={() => sendAppointmentLetter(params.row)}>Send Letter</Button>
  ...
  return (
    <MenuPageShell title="Examiner Allotment 2">
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            <Box>
              <Typography variant="h5" fontWeight={900}>Examiner Allotment 2</Typography>
        ...
        {form.coursecode && !hasCourseSpecificExaminers && (
          <Alert severity="info" sx={{ mb: 2 }}>
            No examiners are pre-registered specifically for <strong>{form.course || form.coursecode}</strong> under this exam. All institutional faculty and registered examiners are available in the dropdown below (they will be auto-linked to this course upon allotment), or you can pre-register dedicated examiners in <RouterLink to="/conduct-exam-2-examiner-list" style={{ fontWeight: 600, color: "inherit", textDecoration: "underline" }}>Examiner List</RouterLink>.
          </Alert>
        )}
        ...
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={4}>
              <Autocomplete
                options={availableExaminers}
                groupBy={(option) => option.category}
                getOptionLabel={(option) => `${option.examinername || ""}${option.examineremail ? ` (${option.examineremail})` : ""}`}
                isOptionEqualToValue={(option, value) => (option?.examineremail || "").toLowerCase() === (value?.examineremail || "").toLowerCase()}
                onChange={(event, value) => selectExaminer(value)}
                renderInput={(params) => <TextField {...params} label="Examiner for Manual Entry" />}
              />
            </Grid>
            <Grid item xs={12} md={4}>

...
export default function ConductExam2ReviewPapersPage() {
  ...
  const loadOptions = async () => {
    const res = await ep1.get("/api/v2/conductexam2/review-options", { params: { colid: global1.colid } });
  ...
  const loadPapers = async (nextFilters = filters) => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/review-papers", { params });
  ...
  const loadDetails = async (paperid) => {
    ...
    try {
      setLoading(true);
      setError("");
      const res = await ep1.get("/api/v2/conductexam2/review-paper-details", { params: { colid: global1.colid, paperid } });
  ...
  const uploadReviewDocument = async (file) => {
    ...
    try {
      ...
      const uploadRes = await ep1.post("/api/v2/conductexam2/question-paper-upload", formData, { headers: { "Content-Type": "multipart/form-data" } });
      ...
      const saveRes = await ep1.post("/api/v2/conductexam2/question-paper-documents", { colid: global1.colid, paperid: paper._id, target: "reviewdocuments", documents: nextDocs, user: global1.user });
  ...
  const updatePaperStatus = async () => {
    ...
    try {
      ...
      setError("");
      setMessage("");
      const res = await ep1.post("/api/v2/conductexam2/review-paper-status", {
        colid: global1.colid,
        paperids,
  ...
  const acceptPaper = async () => {
    ...
    try {
      ...
      setError("");
      setMessage("");
      const res = await ep1.post("/api/v2/conductexam2/review-paper-accept", { colid: global1.colid, paperids, user: global1.user });
      refreshChangedRows(res.data?.data || []);
  ...
  const storeBlockchain = async () => {
    ...
    try {
      ...
      setError("");
      setMessage("");
      const res = await ep1.post("/api/v2/conductexam2/review-paper-blockchain-store", {
        colid: global1.colid,
        paperids,
        user: global1.user,

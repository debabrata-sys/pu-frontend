...
export default function ConductExam2ExaminerListPage() {
  ...
  const loadOptions = async (params = {}) => {
    const res = await ep1.get("/api/v2/conductexam2/examiner-options", { params: { colid: global1.colid, ...params } });
  ...
  const loadRows = async (nextFilters = filters) => {
    try {
      ...
      const res = await ep1.get("/api/v2/conductexam2/examiners", { params });
  ...
  const deleteRow = async (id) => {
    ...
    await ep1.post("/api/v2/conductexam2/examiners-delete", { id, colid: global1.colid });
  ...
  const buildExaminerOrderHtml = (items) => {
    ...
    items.forEach((row) => {
      ...
      if (!groups.has(key)) groups.set(key, { examinername: row.examinername || "", examineremail: row.examineremail || "", examinercode: row.examinercode || "", rows: [] });
    ...
    const sections = [...groups.values()].map((group, index) => `
      <section class="order ${index ? "page-break" : ""}">
        ...
        <div class="meta">
          ...
          ${group.examinercode ? `<div><strong>Examiner Code:</strong> ${htmlEscape(group.examinercode)}</div>` : ""}
        </div>
          ...
          <tbody>
            ${group.rows.map((row) => `
              <tr>
                ...
                <td>${htmlEscape(row.examinercode || "")}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
  ...
  const columns = [
    ...
    { field: "examinercode", headerName: "Examiner Code", width: 150 },
          ...
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", md: "center" }} spacing={1.5} sx={{ mb: 1.5 }}>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap">
              <Button
                ...
                onClick={() => navigate("/conduct-exam-2-examiner-letter")}
                sx={{ fontWeight: 700 }}
              >

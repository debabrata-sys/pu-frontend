...
export default function ConductExamModeratorRegistrationPage() {
  ...
  const saveModerator = async () => {
    try {
      ...
      if (!editId && selectedUsers.length) {
        ...
        const res = await ep1.post("/api/v2/conductexam/moderators", { ...base, id: editId });
  ...
  const sendAppointmentLetter = async (row) => {
    try {
      ...
      const res = await ep1.post("/api/v2/conductexam/moderators-send-letter", { colid: global1.colid, id: row._id });
      if (res.data?.success) {

...
export default function FeedbackInternalManagement() {
  ...
  const fetchFeedbacks = async () => {
    ...
    try {
      ...
      const res = await ep1.get("/api/v2/getallfeedbacksinternalds", { params });
  ...
  const handleDelete = async () => {
    try {
      await ep1.get("/api/v2/deletefeedbackinternalds", {
  ...
  const generateShareLink = (feedbackId) => {
    return `${window.location.origin}/feedbackinternalresponse/${feedbackId}`;
  };
  ...
  const handleCreateNew = () => {
    navigate('/createfeedbackinternal', { 
      ...
      renderCell: ({ row }) => (
        <Box display="flex" gap={0.5}>
          <IconButton 
            size="small" 
            color="primary"
            onClick={() => navigate(`/editfeedbackinternal/${row._id}`)}
          >
            <Edit />
          </IconButton>
          <IconButton 
          ...
          <IconButton 
            size="small" 
            color="info"
            onClick={() => navigate(`/feedbackinternalresponses/${row._id}`)}
          >
            <People />
          </IconButton>
          <IconButton 
            size="small" 
            color="success"
            onClick={() => navigate(`/feedbackinternalanalytics/${row._id}`)}
          >
            <Analytics />
          </IconButton>
          <IconButton 

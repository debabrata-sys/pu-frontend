...
export default function ConductExam2ExaminerMarksEntryPage() {
  ...
  const loadPapers = async () => {
    ...
    try {
      ...
      setError("");
      setMessage("");
      const res = await ep1.get("/api/v2/conductexam2/examiner-marks-papers", {
  ...
  const loadComponents = async (paper) => {
    try {
      setLoading(true);
      setError("");
      const res = await ep1.get("/api/v2/conductexam2/examiner-marks-components", {
        params: {
          colid: global1.colid,
  ...
  const loadStudents = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const res = await ep1.get("/api/v2/conductexam2/examiner-marks-students", {
        params: {
          colid: global1.colid,
  ...
  const saveMarks = async () => {
    ...
    try {
      ...
      setError("");
      setMessage("");
      const res = await ep1.post("/api/v2/conductexam2/examiner-marks-save", {
        colid: global1.colid,
        user: examinerEmail,
        paper: selectedPaper,
  ...
  return (
    <MenuPageShell title="Examiner Marks Entry 2">
      <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "#f6f7fb", minHeight: "100vh" }}>
        <Paper elevation={0} sx={{ p: 2.5, mb: 2, border: "1px solid #e5e7eb", borderRadius: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}>
            <Box>
              <Typography variant="h5" fontWeight={900}>Examiner Marks Entry 2</Typography>

...
export default function ConductExamRemunerationPaymentPage() {
  ...
  const handleGenerateBillSubmit = async () => {
    ...
    try {
      ...
      const res = await ep1.post("/api/v2/conductexam/remuneration/generate-bill", payload);
      if (res.data?.success) {
        setMessage(`Bill #${res.data.data.billno} generated successfully!`);
        setBillModalOpen(false);
        loadPaymentData();
        // Redirect or open print view
        navigate(`/conduct-exam-remuneration-bill/${res.data.data._id}`);
      }
    } catch (err) {
  ...
  const handlePrintBillReport = () => {
    ...
    navigate(`/conduct-exam-remuneration-report?${params.toString()}`);
  };
